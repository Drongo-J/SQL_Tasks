...
BEGIN
	...
	SELECT @book_id=inserted.Id_Book FROM inserted

	DECLARE @book_quantity AS INT

	SELECT 
		@book_quantity=B.Quantity 
	FROM
		Books AS B
	WHERE
		B.Id=@book_id
	...
	IF(@book_quantity = 0)
	BEGIN
		PRINT 'You cannot take book. Book quantity : 0'
		ROLLBACK TRANSACTION
	END
	--ELSE
	--BEGIN
	--	PRINT 'Book Added Successfully To S_Cards'
	--	UPDATE 
	--		Books 
	--	SET 
	--		Quantity = Quantity - 1 
...
BEGIN
	...
	--ELSE
	--BEGIN
	--	PRINT 'Book Added Successfully To T_cards'
	--	UPDATE 
	--		Books 
	--	SET 
	--		Quantity = Quantity - 1 
...
INSERT INTO S_Cards([Id], [DateIn],[DateOut],[Id_Book],[Id_Lib],[Id_Student])
VALUES (123,'1-1-2001', '2-2-2002',3, 1, 2)
...
GO
CREATE TRIGGER StudentReturnBook_tr
ON S_cards
AFTER DELETE 
AS 
BEGIN 
	...
	PRINT 'Book Quantity Was Successfully Increased'
END

GO
CREATE TRIGGER TeacherReturnBook_tr
ON T_Cards
AFTER DELETE 
AS 
BEGIN 
...
GO
CREATE TRIGGER StudentTakeBookFromLibrary_tr
ON S_Cards
AFTER INSERT
AS
BEGIN
	...
	PRINT 'Book Quantity Was Successfully Decreased'
END

GO
CREATE TRIGGER TeacherTakeBookFromLibrary_tr
ON T_Cards
AFTER INSERT
AS
BEGIN
...
INSERT INTO S_Cards([Id],[Id_Book],[Id_Lib],[Id_Student],[DateIn],[DateOut])
VALUES (999, 18, 1, 2, '02-23-2006','11-20-2022')
...
GO
CREATE TRIGGER NoBooksMoreThan3ForStudent_tr
ON S_Cards
AFTER INSERT
AS
BEGIN
	...
	SELECT
		@student_id=inserted.Id_Student
	FROM 
		inserted 

	DECLARE @taken_book_count AS INT

	SELECT
		@taken_book_count = COUNT(*)
	FROM
		S_Cards
	WHERE
		Id_Student=@student_id

	IF (@taken_book_count > 3)
	BEGIN
		PRINT 'Student cannot take more than 3 book'
		ROLLBACK TRANSACTION
	END
END
...
INSERT INTO S_Cards([Id],[Id_Book],[Id_Lib],[Id_Student],[DateIn],[DateOut])
VALUES (7737, 1, 1, 6, '2-5-2008','4-2-2024'),
	   (7738, 18, 1, 6, '1-4-2008','5-3-2026')
...
INSERT INTO S_Cards([Id],[Id_Book],[Id_Lib],[Id_Student],[DateIn],[DateOut])
VALUES (7739, 1, 1, 6, '2-3-2006','3-5-2026')
...
GO
CREATE TRIGGER DontIssueBookToStudent_tr
ON S_Cards
AFTER INSERT
AS 
BEGIN
	...
	IF((SELECT 
		...
		FROM
			(SELECT
				DATEDIFF(month,S.DateOut, GETDATE()) AS "Date Difference"
			FROM
				S_Cards AS S
			WHERE
				S.Id_Student= @student_id) AS DD
		WHERE 
			DD.[Date Difference] > 2) > 0)
	BEGIN 
		PRINT 'Student Cannot Take Book. He/She has taken a book for more than 2 month'
		ROLLBACK TRANSACTION
	END
END
...
--Cannot take (student with id 6)
INSERT INTO S_Cards([Id],[Id_Book],[Id_Lib],[Id_Student],[DateIn],[DateOut])
VALUES (4532, 1, 1, 3, '2-5-2008','4-2-2024')
--Can take (student with id 25)
INSERT INTO S_Cards([Id],[Id_Book],[Id_Lib],[Id_Student],[DateIn],[DateOut])
VALUES (4532, 1, 1, 25, '2-5-2008','4-2-2024')
...
CREATE TABLE [dbo].[LibDeleted](
	...
	[Comment] [nvarchar](50) NULL,
	[Quantity] [int] NOT NULL,
 CONSTRAINT [PK_LibDeleted] PRIMARY KEY CLUSTERED 
(
	[Id] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON) ON [PRIMARY]
) ON [PRIMARY]
...
BEGIN
	...
	DECLARE @book_name AS NVARCHAR(MAX)
	SELECT @book_name=[Name] FROM deleted

	DECLARE @pages AS INT
	SELECT @pages=Pages FROM deleted

	DECLARE @yearPress AS INT
	SELECT @yearPress=YearPress FROM deleted

	DECLARE @themeId AS INT
	SELECT @themeId=Id_Themes FROM deleted

	DECLARE @categortyId AS INT
	SELECT @categortyId=Id_Category FROM deleted
	
	DECLARE @authorId AS INT
	SELECT @authorId=Id_Author FROM deleted
	
	DECLARE @pressId AS INT
	SELECT @pressId=Id_Press FROM deleted
	...
	VALUES (@bookId,@book_name,@pages,@yearPress,@themeId,@categortyId,@authorId,@pressId,@comment,@quantity)
END
